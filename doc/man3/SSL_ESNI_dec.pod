...
These are internal functions for handling encrypted SNI. TODO: Details:-) 

Soon(ish), I should move the prototypes for these out of esni.h and into
and esni-int.h file. (That'll need to be included in esnistuff/esnimain.c
as well.) Doing that might force some more code changes to really make
all the right data structures internal though, so it may be a while
before I get there. 
