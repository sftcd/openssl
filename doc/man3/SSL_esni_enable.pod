...
- ESNI_RRFMT_ASCIIHEX: the buffer contains ascii hex encoded ESNIKeys
...
in the DNS.  (For the base64 encoded option, those need to be separated by a
semi-colon.)
...
- SSL_ESNI_STATUS_BAD_NAME  - ESNI succeeded but the server cert didn't match the hidden service name
- SSL_ESNI_STATUS_TOOMANY  - ESNI succeeded but we can't figure out which one!

SSL_esni_query() provide a view of some of the (non-cryptorgraphic) content
contained within a set of ESNI keys.  This allows a client wishes to select one
of a set of ESNI keys via SSL_ESNI_reduce(). The structure returned by
SSL_esni_query() is as shown below:
...
SSL_set_esni_callback() allows the application to set a callback that will
print ESNI information for debugging purposes. On clients, that is called when
the ESNI extension is added to the ClientHello. On servers, this is called at
the end of successful decryption of a received ESNI extension.

SSL_CTX_set_esni_callback() sets the callback in the context, and that is
copied into derived SSL sessions.
