...
emit not outer SNI value at all then it should set the I<no_outer> to 1
...
be smaller than an outer ClientHello. If decrytpion succeds then the
...
SSL_ech_get_info() returns an array of B<OSSL_ECH_INFO> structures. 
...
additional details of GREASEing. B<MORE TBD>
...
This allows client to GREASE with future extension type codepoints if needed.
