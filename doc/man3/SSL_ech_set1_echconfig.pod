...
provide a binary, ascii-hex or base64 encoded ECHConfigList in the I<val> (of
size I<len>).  If all is well, the ECHConfig values (containing ECH public
values) will be associated with the SSL connection I<s> and ECH will be
attempted when a TLS session is established.  In the case of
SSL_CTX_ech_set1_echconfig(), the ECH settings will be inherited when a new SSL
connection is formed via L<SSL_new(3)>.

If multiple calls to SSL_ech_set1_echconfig() are made for the same I<SSL> (or
resp. I<SSL_CTX>), then the ECHConfig values will be accumulated.  reflect the
total number of ECH public values available.  SSL_ech_get_info() (see below)
can be used to determine how many ECHConfig public values have been
...
of the I<inner_ch> buffer) and sets I<decrypted_ok> to 1. Note that
