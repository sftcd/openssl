...
https://datatracker.ietf.org/doc/draft-ietf-tls-esni/ and defines a way to
...
L<openssl-s_client>, L<openssl-s_server> and L<openssl-ech>.
...
to use ECH when a new TLS session is negotiated e.g. using L<SSL_connect()>
or L<SSL_write()>. If an ECH public value is associated with an I<SSL>
...
inherited when a new SSL connection is formed via L<SSL_new()>.
...
reflect the total number of ECH public values available.
...
ClientHello (e.g. as provided to L<SSL_set_tlsext_host_name()>) is used as the
...
Typically, applications control ALPN values via L<SSL_CTX_set_alpn_protos()> and
...
handles as for L<SSL_CTX_set_alpn_protos()>.
...
=item - I<suite> is an OSSL_HPKE_SUITE value (OSSL_HPKE_SUITE_DEFAULT is a good default, see L<OSSL_HPKE_CTX_new> for details)
...
ECH configuration data, such as L<openssl-ech>.
...
used is specified in https://datatracker.ietf.org/doc/draft-farrell-tls-pemesni/
...
if the I<inner_ch> buffer). Note that an inner ClientHello will always
be smaller than an outer ClientHello. The encoding of both I<outer_ch>
and I<inner_ch> begins with the TLS message type and 3-octet length
(i.e. the initials octet are [0x16, 0x03, 0x01] followed by a 2-octet length).

If decrytpion succeeds in SSL_CTX_ech_raw_decrypt() then the
I<decryption_ok> value will be returned as 1. If decryption fails
the I<decryption_ok> value will be returned as 0. If decryption
...
L<OSSL_HPKE_CTX_new> for details of string encoding of HPKE suites. This allows
...
Applications can set a callback function that will be called when a real
attempt at ECH has succeeded. The callback won't be called if ECH fails.
...
that describes the state of ECH. The application should not modify the
string value which is intended for deugging/logging.

An example string I<str> as seen on a client might be:

 *** SSL_ech_print ***
 SSL_ech_print: s=0x52c2340
 SSL_ech_print: inner_s=0x0
 SSL_ech_print: outer_s=0x52f4210
 SSL_ech_print: ech_attempted=1
 SSL_ech_print: ech_attempted_type=0xfe0d
 SSL_ech_print: ech_atttempted_cid=0x66
 SSL_ech_print: ech_done=1
 SSL_ech_print: ech_grease=0
 SSL_ech_print: HRR=0
 SSL_ech_print: hrr_depth=-1
 SSL_ech_print: ech_returned=0x0
 SSL_ech_print: ech_returned_len=0
 SSL_ech_print: ech_backend=0
 SSL_ech_print: ech_success=1
 SSL_ech_print: 1 ECHConfig value loaded
 SSL_ech_print: cfg(0): [fe0d,66,cloudflare-esni.com,0020,[0001,0001],5c6559512bba3150800aa5fb4c88b6d4b6e7e1a57f2f9b0cba409b6e474b874d,00,00]
