...
the names of private and public key files, or the name of a file that contains
both keys. The private key file content must be in PEM format. The content of
the public key file must be the binary form of an ESNIKeys RR value, as would
be published in the DNS. Public keys can follow drafts -02, -03 or -04 of the
specification.

If both keys are provided in one file, then both private key and public key
must be in PEM format. In that case the esnipubfile must be NULL. 
