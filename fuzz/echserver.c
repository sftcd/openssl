 ...
 * to the outer CH as an ECH extension
 * For now, there's no attempt to ensure any other 
 * formatting is correct, we'll see how the fuzzer gets
 * on with finding those itself first.
 */
                            ...
                            const unsigned char *in, size_t inlen)
{
    ...
    size_t halflen = inlen / 2;
    const unsigned char *pt = NULL;

    /* make sure there's one octet to process somewhere */
    if (halflen == 0) {
        halflen++;
        pt = in;
    } else {
    ...
    ctlen = OSSL_HPKE_get_ciphertext_size(hpke_suite, halflen);
    echlen = 14 + mypublen + ctlen;
    choutlen = halflen + echlen;
    chout = OPENSSL_malloc(choutlen);
    if (chout == NULL)
        goto err;
    cp = chout;
    /* outer CH from fuzzer */
    memcpy(cp, in, halflen);
    cp += halflen;
    *cp++ = 0xfe;
    *cp++ = 0x0d;
    *cp++ = (echlen >> 8) & 0xff;
    ...
    *cp++ = ctlen & 0xff;
    ...
    if (OSSL_HPKE_seal(hctx, ct, &ctlen, aad, aadlen, pt, halflen) != 1)
        goto err;
    *out = chout;
    *outlen = choutlen;
    res = 1;
...
# endif
    ...
    if ((opt & 0x02) != 0) {
        ...
        /* otherwise add ECHConfig/priv to ctx, add fuzz CH */
