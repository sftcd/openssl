...
char *psk_key_sech = NULL;           /* by default PSK is not used */
                                  ...
                                  unsigned int max_psk_len)
{
    ...
    key = OPENSSL_hexstr2buf(psk_key_sech, &key_len);
    if (key == NULL) {
...
const OPTIONS s_server_sech_options[] = {
    OPT_SECTION("General"),
...
int s_server_sech_main(int argc, char *argv[])
{
    ENGINE *engine = NULL;
 ...
 opthelp:
        ...
        case OPT_PSK_HINT:
            ...
            break;
        case OPT_PSK:
            for (p = psk_key_sech = opt_arg(); *p; p++) {
                if (isxdigit(_UC(*p)))
                    continue;
...
# endif
    ...
    if (psk_key_sech != NULL || psksess != NULL)
